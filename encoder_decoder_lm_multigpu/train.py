...
class Trainer:
    ...
    def train(
        ...
        self.train_loader, self.val_loader, self.test_loader, self.model, optimizer = accelerator.prepare(
            self.train_loader, self.val_loader, self.test_loader,
            self.model, optimizer,
        )
        #### accelerator code ends
        ...
        for epoch in range(num_epochs):
            ...
            self._runtime_evaluate(self.val_loader, accelerator)

            self._save_model()
            
        print(f"Total Time: {total_time} (s)")
        self._runtime_evaluate(self.test_loader, accelerator)
        
        accelerator.end_training()
    
    def _runtime_evaluate(self, dataset, accelerator):
        self.model.eval()
        # for summarization
        m_rouge1 = 0
        m_rouge2 = 0
